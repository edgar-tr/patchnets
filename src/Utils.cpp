...
std::vector<Eigen::Vector4f> ValidPointsAndTrisFromIm(
  ...
  for (unsigned int w = 0; w < pixNorms.w; w++) {
    for (unsigned int h = 0; h < pixNorms.h; h++) {
      ...
      points.push_back(n);
	  point_normal_goodness.push_back(goodObs);
    }
  }
  return points;
}
...
float BoundingCubeNormalization(
  ...
  normalization_parameters.push_back(xCenter);
  normalization_parameters.push_back(yCenter);
  normalization_parameters.push_back(zCenter);
  ...
  // add some buffer
  maxDistance *= buffer;
  normalization_parameters.push_back(maxDistance);
