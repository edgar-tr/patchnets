...
void SampleSDFNearSurface(
    ...
    int num_votes,
	std::vector<Eigen::Vector3f>& closest_normals,
	std::vector<bool>& normal_goodness,
	std::vector<bool>& point_normal_goodness) {
  ...
  for (int s = 0; s < (int)xyz.size(); s++) {
    ...
    if ((num_pos == 0) || (num_pos == num_votes)) {
	  ...
	  uint32_t closest_index = cl_indices[0];
	  closest_normals.push_back(normals[closest_index]);
      normal_goodness.push_back(point_normal_goodness[closest_index]);
      //normal_goodness.push_back(true);
    }
  }
...
void writeSDFToNPZ(
	...
	float wrong_ratio,
	float bad_tri_ratio,
	std::vector<float>& normalization_parameters,
	std::vector<float>& frontal_depth,
	bool store_frontal_depth,
	std::vector<float>& random_depth_0,
	std::vector<float>& random_depth_1,
	std::vector<float>& random_depth_0_modelview,
	std::vector<float>& random_depth_1_modelview,
	bool store_random_depth,
	size_t width,
	size_t height,
    std::string filename,
  ...
  std::vector<float> pos_normals;
  std::vector<float> neg_normals;
  std::vector<float> pos_goodness;
  std::vector<float> neg_goodness;

  for (unsigned int i = 0; i < num_vert; i++) {
	Eigen::Vector3f v = xyz[i];
    float s = sdfs[i];
	Eigen::Vector3f normal = closest_normals[i];
    ...
    if (s > 0) {
      ...
      pos.push_back(s);
	  for (int j = 0; j < 3; j++)
		pos_normals.push_back(normal[j]);
	  pos_goodness.push_back(normal_goodness[i] ? 1.0f : 0.0f);
    } else {
      for (int j = 0; j < 3; j++)
        neg.push_back(v[j]);
      neg.push_back(s);
	  for (int j = 0; j < 3; j++)
		neg_normals.push_back(normal[j]);
	  neg_goodness.push_back(normal_goodness[i] ? 1.0f : 0.0f);
    }
  }
  ...
  cnpy::npz_save(filename, "pos_normals", &pos_normals[0], {(long unsigned int)(pos_normals.size() / 3.0), 3}, "a");
  cnpy::npz_save(filename, "neg_normals", &neg_normals[0], { (long unsigned int)(neg_normals.size() / 3.0), 3 }, "a");
  cnpy::npz_save(filename, "pos_goodness", &pos_goodness[0], { (long unsigned int)(pos_goodness.size() / 1.0), 1 }, "a");
  cnpy::npz_save(filename, "neg_goodness", &neg_goodness[0], { (long unsigned int)(neg_goodness.size() / 1.0), 1 }, "a");
  cnpy::npz_save(filename, "bad_obs_ratio", &wrong_ratio, { 1, 1 }, "a");
  cnpy::npz_save(filename, "bad_tri_ratio", &bad_tri_ratio, { 1, 1}, "a");
  cnpy::npz_save(filename, "normalization_parameters", &normalization_parameters[0], { (long unsigned int)(normalization_parameters.size() / 1.0), 1 }, "a");
  if (store_frontal_depth)
	cnpy::npz_save(filename, "depth", &frontal_depth[0], {height, width}, "a");
  if (store_random_depth)
  {
	cnpy::npz_save(filename, "random_depth_0", &random_depth_0[0], {height, width}, "a");
	cnpy::npz_save(filename, "random_depth_0_modelview", &random_depth_0_modelview[0], {4, 4}, "a");
	cnpy::npz_save(filename, "random_depth_1", &random_depth_1[0], {height, width}, "a");
	cnpy::npz_save(filename, "random_depth_1_modelview", &random_depth_1_modelview[0], {4, 4}, "a");
  }
  if (print_num) {
...
int main(int argc, char** argv) {
  ...
  bool store_frontal_depth = false;
  bool store_random_depth = false;
  ...
  app.add_flag("--frontal_depth", store_frontal_depth, "include frontal depth in npz");
  app.add_flag("--random_depth", store_random_depth, "include random depth in npz");
  ...
  pangolin::Geometry geom = pangolin::LoadGeometry(meshFileName); // can take about a minute
  ...
  std::vector<float> normalization_parameters;
  float max_dist = BoundingCubeNormalization(geom, true, normalization_parameters);
  ...
  int default_num_views = 100;
  std::vector<Eigen::Vector3f> views = EquiDistPointsOnSphere(default_num_views, max_dist * 1.1);
  if (store_frontal_depth)
  {
	  Eigen::Vector3f front_view(1.0f, 1.0f, -1.0f);
	  views.push_back(front_view); // final view is from front. only store this view, don't use it for any preprocessing
  }
  if (store_random_depth)
  {
	  std::random_device rd{};
	  ...
	  Eigen::Vector3f random_view(d(gen), d(gen), d(gen));
	  views.push_back(random_view);
	  Eigen::Vector3f random_view_1(d(gen), d(gen), d(gen));
	  views.push_back(random_view_1);
  }
  ...
  for (unsigned int v = 0; v < views.size(); v++) {
	  ...
	  bool store_this_depth = v >= default_num_views;
	  //bool store_this_depth = store_frontal_depth && v == views.size() - 1;
	  ...
	  prog.SetUniform("outputCameraVertex", (store_this_depth ? 1 : 0));
	  pangolin::GlDraw(prog, gl_geom, nullptr);
	  prog.Unbind();
	  ...
	  if (!store_this_depth)
	  {
		  ...
		  std::vector<Eigen::Vector4f> im_norms = ValidPointsAndTrisFromIm(
			  img_normals.UnsafeReinterpret<Eigen::Vector4f>(), tri_id_normal_test, total_obs, wrong_obs, point_normal_goodness);
	  ...
	  if (store_this_depth)
	  {
		  const pangolin::Image<Eigen::Vector4f> verts = img_verts.UnsafeReinterpret<Eigen::Vector4f>();
		  Eigen::Vector4f current_depth_pixel;
		  float to_store;
		  for (unsigned int y = 0; y < verts.h; y++)
		  {
			  for (unsigned int x = 0; x < verts.w; x++)
			  {
				  current_depth_pixel = verts(x, y);
				  if (current_depth_pixel[3] == 0.0f)
				  {
					  to_store = 2.0f; // invalid depth value
				  }
				  else
				  {
					  to_store = current_depth_pixel[2];
				  }
				  if (store_frontal_depth)
				  {
					  if (v == default_num_views)
						  frontal_depth.push_back(to_store);
					  else if (v == default_num_views + 1)
					  {
						  random_depth_0.push_back(to_store);
						  for (unsigned int i = 0; i < 4; i++)
							  for (unsigned int j = 0; j < 4; j++)
								  random_depth_0_modelview.push_back(s_cam2.GetModelViewMatrix()(i, j));
					  }
					  else
					  {
						  random_depth_1.push_back(to_store);
						  for (unsigned int i = 0; i < 4; i++)
							  for (unsigned int j = 0; j < 4; j++)
								  random_depth_1_modelview.push_back(s_cam2.GetModelViewMatrix()(i, j));
					  }
				  }
				  else {
					  if (v == default_num_views)
  ...
  std::cout << "kdTree " << elapsed << std::endl;
  ...
  int num_samp_near_surf = (int)(47 * (long) num_sample / 50);
  std::cout << "num_samp_near_surf: " << num_samp_near_surf << std::endl;
  SampleFromSurface(geom, xyz_surf, num_samp_near_surf / 2);

  finish = std::chrono::high_resolution_clock::now();
  elapsed = std::chrono::duration_cast<std::chrono::seconds>(finish - start).count();
  std::cout << "SampleFromSurface " << elapsed << std::endl;
  ...
  SampleSDFNearSurface(
      ...
      closest_normals,
      normal_goodness,
      point_normal_goodness);

  finish = std::chrono::high_resolution_clock::now();
  elapsed = std::chrono::duration_cast<std::chrono::seconds>(finish - start).count();
  std::cout << "SampleSDFNearSurface " << elapsed << std::endl;
  ...
  if (save_npz == std::string::npos)
  {
    writeSDFToNPY(xyz, sdf, npyFileName);
      ...
      writeSDFToNPZ(xyz, sdf, closest_normals, normal_goodness, wrong_ratio, bad_tri_ratio, normalization_parameters, frontal_depth, store_frontal_depth, random_depth_0, random_depth_1, random_depth_0_modelview, random_depth_1_modelview, store_random_depth, w, h, npyFileName, true);
  }
  return 0;
}
