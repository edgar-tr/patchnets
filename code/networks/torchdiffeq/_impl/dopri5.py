...
class Dopri5Solver(AdaptiveStepsizeODESolver):
    ...
    def __init__(
        self, func, y0, conditioning, rtol, atol, first_step=None, safety=0.9, ifactor=10.0, dfactor=0.2, max_num_steps=1000, total_remaining_forced_step=10,#2**31 - 1,
        **unused_kwargs
    ):
        _handle_unused_kwargs(self, unused_kwargs)
        del unused_kwargs

        self.total_remaining_forced_steps = total_remaining_forced_step # how many steps to take after running out of max_num_steps in order to force the integration to stop

        self.func = func
        self.y0 = y0
        self.conditioning = conditioning if type(conditioning) == type((0,)) else (conditioning,)
    ...
    def before_integrate(self, t):
        ...
        f0 = self.func(t[0].type_as(self.y0[0]), self.y0, self.conditioning)
        if self.first_step is None:
            first_step = _select_initial_step(self.func, t[0], self.y0, self.conditioning, 4, self.rtol[0], self.atol[0], f0=f0).to(t)
        else:
            first_step = _convert_to_tensor(self.first_step, dtype=t.dtype, device=t.device)
        self.rk_state = _RungeKuttaState(self.y0, self.conditioning, f0, t[0], t[0], first_step, interp_coeff=[self.y0] * 5)

    def advance(self, next_t):
        """Interpolate through the next time point, integrating as necessary."""
        n_steps = 0
        remaining_forced_steps = None # if max_num_steps is reached, force the integration to step in at most remaining_forced_steps

        while next_t >= self.rk_state.t1:
            #assert n_steps < self.max_num_steps, 'max_num_steps exceeded ({}>={})'.format(n_steps, self.max_num_steps)
            if n_steps >= self.max_num_steps:
                if remaining_forced_steps is None:
                    print("too many integration steps (" + str(n_steps) + "), stopping integration early...")
                    remaining_forced_steps = self.total_remaining_forced_steps
                else:
                    remaining_forced_steps -= 1
            self.rk_state, accept_step = self._adaptive_dopri5_step(self.rk_state, remaining_forced_steps, next_t, since_last_accept)
            n_steps += 1
        #print("number of steps: " + str(n_steps), flush=True)
        #print("number of accepted steps: " + str(accepted), flush=True)
        return _interp_evaluate(self.rk_state.interp_coeff, self.rk_state.t0, self.rk_state.t1, next_t)

    def _adaptive_dopri5_step(self, rk_state, remaining_forced_steps, next_t, since_last_accept):
        # next_t is usually not an argument to this function. it's only used to force the integration to stop
        """Take an adaptive Runge-Kutta step to integrate the ODE."""
        y0, cond0, f0, _, t0, dt, interp_coeff = rk_state
        ########################################################
        #                      Assertions                      #
        ########################################################
        ...
        y1, f1, y1_error, k = _runge_kutta_step(self.func, y0, cond0, f0, t0, dt, tableau=_DORMAND_PRINCE_SHAMPINE_TABLEAU)
        ...
        if remaining_forced_steps is None:
            ...
            eps = 0.0000000001
            dt_next = ((next_t + eps) - t0) / torch.max(torch.tensor([1., remaining_forced_steps], device=y0[0].device))

        rk_state = _RungeKuttaState(y_next, cond0, f_next, t0, t_next, dt_next, interp_coeff)
        return rk_state, accept_step
