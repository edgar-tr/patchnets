...
class _RungeKuttaState(collections.namedtuple('_RungeKuttaState', 'y1, cond1, f1, t0, t1, dt, interp_coeff')):
...
def _runge_kutta_step(func, y0, conditioning, f0, t0, dt, tableau):
    ...
    for alpha_i, beta_i in zip(tableau.alpha, tableau.beta):
        ...
        tuple(k_.append(f_) for k_, f_ in zip(k, func(ti, yi, conditioning)))
...
def rk4_alt_step_func(func, t, dt, y, conditioning, k1=None):
    """Smaller error with slightly more compute."""
    if k1 is None: k1 = func(t, y, conditioning)
    k2 = func(t + dt / 3, tuple(y_ + dt * k1_ / 3 for y_, k1_ in zip(y, k1)), conditioning)
    k3 = func(t + dt * 2 / 3, tuple(y_ + dt * (k1_ / -3 + k2_) for y_, k1_, k2_ in zip(y, k1, k2)), conditioning)
    k4 = func(t + dt, tuple(y_ + dt * (k1_ - k2_ + k3_) for y_, k1_, k2_, k3_ in zip(y, k1, k2, k3)), conditioning)
