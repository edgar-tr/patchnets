...
def main(args):
    ...
    input_files = [input_file for input_file in input_files if not check_if_mesh_exists_already((input_file, args))]
    
    if args.n_proc != 0:
        with Pool(args.n_proc) as p:
            #p.map(partial(process_path, args=args), input_files)
            p.map(process_path, [(input_file, args) for input_file in input_files])
    else:
    ...
    if not args.resize:
        ...
        try:
            if args.bbox_in_folder is not None:
                in_path_tmp = os.path.join(args.bbox_in_folder, modelname + '.obj')
                mesh_tmp = trimesh.load(in_path_tmp, process=False)
                mesh_tmp = trimesh.util.concatenate(mesh_tmp)
        ...
        scale = (max_corner - min_corner).max() / (1 - args.bbox_padding)
...
def check_if_mesh_exists_already(incom):
    in_path, args = incom
    in_file = os.path.basename(in_path)
    modelname = os.path.splitext(in_file)[0]
    filename = os.path.join(args.mesh_folder, modelname + '.obj')    
    return not args.overwrite and os.path.exists(filename)
